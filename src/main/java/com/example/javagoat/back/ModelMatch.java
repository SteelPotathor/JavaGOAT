...
public class ModelMatch implements Serializable {
    ...
    public HashMap<Profile, Integer> getKNN(int noProfile, int howMany) {
        ...
        HashMap<Integer, Date> hashMapH = ModelProfile.profileHashMap.get(noProfile).getModelHisto().getStockHisto();
        ...
        while (i < howMany && itr.hasNext()) {
            ...
            if ((hashMapH == null || !hashMapH.containsKey(t.id)) && t.id != noProfile && ModelProfile.profileHashMap.get(t.id).getPreferences().getBiology().getBsex() == modelP.getProfileHashMap().get(noProfile).getIdentity().getBsex()) {
                HashSet<Passion.miscellaneous> PMprofile = ModelProfile.profileHashMap.get(noProfile).getPassion().getPassionM();
                HashSet<Passion.video_games> PVGprofile = ModelProfile.profileHashMap.get(noProfile).getPassion().getPassionVG();
                int counter = ModelProfile.profileHashMap.get(t.id).getPassion().getPassionM().stream().filter(PMprofile::contains).toArray().length;
                counter += ModelProfile.profileHashMap.get(t.id).getPassion().getPassionVG().stream().filter(PVGprofile::contains).toArray().length;
                KNNProfiles.put(ModelProfile.profileHashMap.get(t.id), counter);
                i++;
            }
